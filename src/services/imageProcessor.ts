...
// of bare (white) sticker material. Operates on raw RGBA.
// ---------------------------------------------------------------------------

/**
 * Grows opaque colours outward by `radius` px. Each newly-filled pixel takes the
 * average colour of its already-filled neighbours, layer by layer, so the colour
 * spreads from the nearest edge. Only the OUTER (border-connected) transparent
 * region is filled — interior transparent holes are left untouched.
 *
 * @param rgba   Raw RGBA buffer (width*height*4), non-premultiplied.
 * @returns      A new RGBA buffer of the same dimensions with the bleed applied.
 */
export function bleedColors(rgba: Buffer, width: number, height: number, radius: number): Buffer {
  ...
  // filled[p] = pixel currently has colour (original opaque, or already bled)
  const filled = new Uint8Array(N);
  for (let p = 0; p < N; p++) filled[p] = rgba[p * 4 + 3] !== 0 ? 1 : 0;

  // outside[p] = transparent pixel reachable from the image border (not an
  // interior hole). Flood fill through transparent pixels from the edges.
  ...
  // Layered dilation: each pass fills the transparent-outside pixels that border
  // the current filled region, using colours captured before the pass.
